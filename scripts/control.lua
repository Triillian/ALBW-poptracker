...
function advanced_misery_mire_oob()
    ...
    if hasAll({ "irod", "trod", "frod" }) then return true end
    if hasAll({ "irod", "trod" }) and crack_clip() then return true end
    if hasAll({ "cracksanity", "frod" }) and hasAny({ "crack_mire_sw", "crack_mire_middle" }) then return true end
    if hasAll({ "cracksanity" }) and hasAny({ "crack_mire_sw", "crack_mire_middle" }) and crack_clip() then return true end



    if (access_misery_mire() and hasAll({ "irod", "trod" }) or (has("merge") and ((has("cracksanity") and hasAny({ "crack_mire_sw", "crack_mire_middle" })) or
            (hasAll({ "not_cracksanity", "quake" }) and (hasAny({ "srod", "scroll" }) or boost()))))) and (boost() or (has("trod") and crack_clip())) then
        return true
    end
...
-- Return if we have any cracks in Central Lorule, including the Lorule Castle Crack
...
function can_reach_final_boss()
    return has("merge") and yg_requirement() and ((has("crack_lc")) or (lc_requirement() and (has("trials_skipped") or hasAll({ "merge", "hookshot" }))))
end
